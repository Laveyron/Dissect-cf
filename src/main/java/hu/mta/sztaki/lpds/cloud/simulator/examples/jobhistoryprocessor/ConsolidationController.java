...
public class ConsolidationController {
	...
	// Maximum number of property files to generate
	private final int MAX_PROPERTY_FILES = 200;
	// Directory to store property files
	private final String PROPERTY_FILES_DIR = "data";
	...
	private void saveProps(String consType, boolean noWrite) {
		if (noWrite)
			return;
		// Skip if we've already generated the maximum number of property files
		if (propCounter >= MAX_PROPERTY_FILES) {
			propCounter++; // Still increment counter to track how many would have been generated
			return;
		}
		try {
			// Create the data directory if it doesn't exist
			File dataDir = new File(PROPERTY_FILES_DIR);
			if (!dataDir.exists()) {
				dataDir.mkdirs();
			}
			
			// Save file to the data directory
			File outputFile = new File(dataDir, consType + "-consolidationProperties" + propCounter++ + ".xml");
